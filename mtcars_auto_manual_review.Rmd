...
Methods for analysis include, exploratory data analysis, the use of liner Regression Models and statistical inference.  Images, tables and other output will be located in the Appendix section and referenced as needed.  The R code and numerous other output has been suppressed for this report, however, if the review is interested it can be accessed using this link: [Report Rmd file](https://github.com/tghuntCoursera/regression_models/blob/master/mtcars_auto_manual_review.Rmd)
...
Scanning the data set also shows that, among other differences, the number of gears and rear axle ratio vary across subjects.  Gearing and axle ration combinations are typically dependent upon the use application for the vehicle and also have an affect on mpg.  Also, as previously noted, aerodynamic drag coefficients are not present in the data set.


### Linear Models
For completeness, some linear models were also investigated.  Figure 3 (***see appendix***) shows the results from simple linear models of (mpg~am) in a table.  From this we can compare the results of all the data to each cylinder group.  Using a simple model the slopes are all positive indicating an increase in mpg when using a manual transmission instead of an automatic.  Hoever, the slopes decrease as the number of subjects with a manual transmission decreases in each group.
